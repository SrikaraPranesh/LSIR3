...
function plotdatabc(n1,n2,n3,svnm,kappa,u,showleg,xmax)

% PLOTDATAABC creates a plot for the three experiments whose output data is 
% stored in MATLAB data objects n1, n2, and n3. 
%
% Input
%       n1:         .mat file name for output of LSIR run
%       n2:         .mat file name for output of G-LSIR run
%       n3:         .mat file name for output of G-LSIR BD run (block diag)
...
xerr2 = G.(SNG{1}).xerr;
rerr2 = G.(SNG{1}).rerr;
xerr3 = G2.(SNG2{1}).xerr;
rerr3 = G2.(SNG2{1}).rerr;

% Generate plot
f = figure();
semilogy(0:numel(xerr1)-1, xerr1, 'bo-','MarkerSize',10,'LineWidth',1);
hold on
semilogy(0:numel(rerr1)-1, rerr1, 'rx-','MarkerSize',10,'LineWidth',1);
hold on
semilogy(0:numel(xerr2)-1, xerr2, 'bo--','MarkerSize',10,'LineWidth',1);
hold on
semilogy(0:numel(rerr2)-1, rerr2, 'rx--','MarkerSize',10,'LineWidth',1);
hold on 
semilogy(0:numel(xerr3)-1, xerr3, 'bo:','MarkerSize',10,'LineWidth',1);
hold on
semilogy(0:numel(rerr3)-1, rerr3, 'rx:','MarkerSize',10,'LineWidth',1);
hold on 
semilogy(0:xmax, u.*ones(xmax+1,1), 'k--','MarkerSize',10,'LineWidth',1);

xlabel('refinement step','FontSize',16,'Interpreter','latex')
if (showleg)
legend({'LSIR $x$', 'LSIR $r$', 'G-LSIR $x$', 'G-LSIR $r$','G-LSIR BD $x$', 'G-LSIR BD $r$'},'Interpreter','latex')
end
title(strcat('$\kappa = $ ', num2str(kappa, '%1.e\n')),'FontSize',16,'Interpreter','latex')
