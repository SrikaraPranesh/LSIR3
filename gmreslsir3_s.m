function opdata = gmreslsir3(A,b,precf,precw,precr,iter_max,gtol)
% GMRESLSIR3  GMRES-based iterative refinement in three precisions.
...
%       * double if precr = 2,
%       * quad if precr = 4
%     Uses left preconditioned GMRES M\A with 
%     M = [alpha.*eye(m), Q1*R; R'*Q1', zeros(n)];
% Note: Requires CleveLaboratory and Advanpix multiprecision computing toolbox

% for alpha<<1, forward error of x and r increases. This implementations is 
...
if precw == 1
    L1 = single([alpha.*eye(m), single(Q1*R); single(R'*Q1'), zeros(n)]);
    L2 = (eye(m+n));
    R1 = (eye(m+n));
    R2 = (eye(m+n));
elseif precw == 2
    L1 = double([alpha.*eye(m), double(Q1*R); double(R'*Q1'), zeros(n)]);
    L2 = double(eye(m+n));
    R1 = double(eye(m+n));
    R2 = double(eye(m+n));
else
...
while ~cged
    ...
    elseif precr == 2
        f = double(alpha)*((double(b)-double(rx)))-(double(A)*double(x));
    ...
    if precw == 2
        [d, err, its, flag] = lsqgmres_dq(A,zeros(m+n,1),(Aug_rhs/nrhs),Q1,R,alpha,m+n,1,gtol);
    else
        [d, err, its, ~] = lsqgmres_sd(A,zeros(m+n,1),(Aug_rhs/nrhs),Q1,R,alpha,m+n,1,gtol);
    end
